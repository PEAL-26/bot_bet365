...
async function mensagem() {
    ...
    await telegram.enviarMensagem(analise.mensagem);
    await telegram.enviarMensagem(confirmacao.mensagem);
    await telegram.enviarMensagem(falha.mensagem);
...
async function testar() {
    for (var i = 0; i <= 30; i++) {
        ...
        if (resultado.Mensagem.tipo != 'info') {
            var resultadoTelegram = await telegram.enviarMensagem(resultado.Mensagem.msg);
        }
